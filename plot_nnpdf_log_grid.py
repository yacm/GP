...
def plot_component_1x2(nu_grid, itd_samples, x_grid, pdf_samples, itd_label, pdf_label, itd_ylim, pdf_ymax, out_path):
    ...
    x_margin_left = 0.1 * x_grid.min()
    x_margin_right = 0.02 * (x_grid.max() - x_grid.min())
    pdf_epsilon = 0.1
    ...
    axs[1].set_xlim(x_grid.min() - x_margin_left, x_grid.max() + x_margin_right)
...
def main():
    ...
    q_minus_log = interpolate_replicas(x_orig, q_minus_replicas, x_grid_log)
    q_plus_log = interpolate_replicas(x_orig, q_plus_replicas, x_grid_log)

    real_integrator, imag_integrator = build_itd_integrators(x_grid_log, nu_grid_full)
    real_itd = real_integrator @ q_minus_log
    imag_itd = imag_integrator @ q_plus_log

    real_path = BASE_DIR / "NNPDF_log_grid_real_1x2.pdf"
    imag_path = BASE_DIR / "NNPDF_log_grid_imag_1x2.pdf"
